...
- Publish to HF dataset `Shimo4228/doctrine-corpus` (CC0; mapping `shimo4228/doctrine-corpus` GitHub ↔ `Shimo4228/doctrine-corpus` HF). Local mirror dir: `/Users/shimomoto_tatsuya/MyAI_Lab/hf-datasets/doctrine-corpus/`. Bulk upload contents: corpus splits (`corpus/v0.1.0/{train,valid,pilot}.jsonl` + `manifest.json`), AI-facing docs (`graph.jsonld` + `graph.jsonl` + `llms.txt` + `llms-full.txt`), release metadata (`CITATION.cff` + `CHANGELOG.md` + `.zenodo.json`), and HF-specific `README.md` (dataset card with `configs.data_files` exposing the 3 splits to the Dataset Viewer; curated separately from the GitHub repo README). Post-release sync: regenerate `graph.jsonl` via `jq -c '.["@graph"][]' graph.jsonld > graph.jsonl` then `hf upload Shimo4228/doctrine-corpus <file> <dest> --repo-type dataset`.
